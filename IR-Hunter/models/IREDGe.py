...
class IREDGe(nn.Module):   # Autoencoder
    ...
    def forward(self, x, t=None):
        ...
        x_2d = x.mean(dim=2)
        ae_out = self.encoder(x_2d)
        ls_vec = self.ls(ae_out[3], t)
        ls_map = ls_vec.unsqueeze(-1).unsqueeze(-1)
        target_size = ae_out[2].shape[2:]
        ls_map = F.interpolate(ls_map, size=target_size, mode='bilinear', align_corners=False)
        ls_map = self.reduce(ls_map)
