...
class FFN(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act='relu', drop_path=0.0):
        ...
        self.fc1 = nn.Sequential(
            ...
            nn.BatchNorm3d(hidden_features),  # 改为 BatchNorm3d
        )
        self.act = act_layer(act)
        self.fc2 = nn.Sequential(
            nn.Conv3d(hidden_features, out_features, 1, stride=1, padding=0),
            nn.BatchNorm3d(out_features),  # 改为 BatchNorm3d
        )
...
class Stem(nn.Module):
    ...
    def __init__(self, img_size=256, in_dim=1, out_dim=512, act='relu'):
        super().__init__()
        self.convs = nn.Sequential(
            nn.Conv3d(in_dim, out_dim // 2, 3, stride=2, padding=1),  # 改为 Conv3d
            nn.BatchNorm3d(out_dim // 2),  # 改为 BatchNorm3d
            act_layer(act),
            nn.Conv3d(out_dim // 2, out_dim, 3, stride=2, padding=1),  # 改为 Conv3d
            nn.BatchNorm3d(out_dim),  # 改为 BatchNorm3d
            act_layer(act),
            nn.Conv3d(out_dim, out_dim, 3, stride=1, padding=1),  # 改为 Conv3d
            nn.BatchNorm3d(out_dim),  # 改为 BatchNorm3d
        )
...
class Downsample(nn.Module):
    ...
    def __init__(self, in_dim=1, out_dim=512):
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv3d(in_dim, out_dim, 3, stride=2, padding=1),  # 改为 Conv3d
            nn.BatchNorm3d(out_dim),  # 改为 BatchNorm3d
        )
...
class DeepGCN_3D(torch.nn.Module):
    def __init__(self, opt):
        super(DeepGCN_3D, self).__init__()
        print(opt)
        k = opt.k
        act = opt.act
        norm = opt.norm
        ...
        reduce_ratios = [2, 2, 2, 2]
        dpr = [x.item() for x in torch.linspace(0, drop_path, self.n_blocks)]  # stochastic depth decay rule
        num_knn = [int(x.item()) for x in torch.linspace(k, k, self.n_blocks)]  # number of knn's k
        max_dilation = 49 // max(num_knn)

        self.stem = Stem(out_dim=channels[0], act=act)
        self.pos_embed = nn.Parameter(torch.zeros(1, channels[0], 256 // 4, 256 // 4, 1))  # 5D tensor for Conv3d
        HW = 256 // 4 * 256 // 4
        ...
        # self.prediction = Seq(nn.Conv3d(channels[-1], 1024, 1, bias=True),  # 改为 Conv3d
        #                       nn.BatchNorm3d(1024),  # 改为 BatchNorm3d
        #                       act_layer(act),
        #                       nn.Dropout(opt.dropout),
        #                       nn.Conv3d(1024, opt.n_classes, 1, bias=True))  # 改为 Conv3d
        self.model_init()

        self.up1 = Up(512, 256, scale_factor=8, bilinear=True)
        self.up2 = Up(256, 128, scale_factor=2, bilinear=True)
        self.up3 = Up(128, 4, scale_factor=2, bilinear=True)

    def model_init(self):
        for m in self.modules():
            if isinstance(m, torch.nn.Conv3d):  # 改为 Conv3d
    ...
    def forward(self, inputs):
        inputs = inputs[:, :, :self.out_channels, :, :]
        x = self.stem(inputs) + self.pos_embed
        B, C, D, H, W = x.shape  # 适配 Conv3d
        ...
        logits = inputs.squeeze(1) * logits
        return torch.sum(logits, dim=1)

        # x = F.adaptive_avg_pool3d(x, 1)  # 改为 3D 平均池化
        # return self.prediction(x).squeeze(-1).squeeze(-1).squeeze(-1)  # 适配 3D 输出
...
def pvig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, out_channels=4, drop_path_rate=0.0, **kwargs):
            ...
            self.out_channels = out_channels  # Dimension of out_channels
            self.emb_dims = 1024  # Dimension of embeddings


    opt = OptInit(**kwargs)
    model = DeepGCN_3D(opt)
