...
__all__ = ['psnr', 'ssim', 'nrms', 'emd', 'mae', 'cc']
...
@input_converter(apply_to=('img1', 'img2'))
def cc(img1, img2, crop_border=0):
    assert img1.shape == img2.shape, (
        f'Image shapes are different: {img1.shape}, {img2.shape}.')
    ...
    # 处理常数数组的情况
    if np.var(img1_flat) == 0 and np.var(img2_flat) == 0:
        return 1.0
    elif np.var(img1_flat) == 0 or np.var(img2_flat) == 0:
        return 0.0

    # 计算相关系数并处理NaN
    with np.errstate(invalid='ignore'):
        cc_value = np.corrcoef(img1_flat, img2_flat)[0, 1]

    return cc_value if not np.isnan(cc_value) else 0.0
...
def multi_process_score(out_name=None, threshold=0.0, label_path=None, save_path=None):
    ...
    time.sleep(0.5)
    # 修改
    # process_count = max(1, int(mul.cpu_count() * (1 - psutil.cpu_percent(None) / 100.0)))
    # pool = mul.Pool(process_count)
    ...
    calculated_score_parital = partial(calculated_score, temp_path=temp_path,
                                       label_path=label_path, save_path=save_path, threshold_label=threshold,
                                       preds=preds)
    ...
    print('copying')
    os.system('cp {} {}'.format(os.path.join(temp_path, f'{out_name}'),
                                os.path.join(os.path.join(os.getcwd(), save_path), f'{out_name}')))
...
def roc_prc(save_path):
    ...
    # ----------------------------
    # 新增：计算每个阈值组的 F1 分数，并取平均值
    # 公式：F1 = 2 * (precision * recall) / (precision + recall)
    # 这里 recall 就等于 tpr
    f1_list = []
    for tpr_val, prec_val in zip(tpr_sum_List, precision_sum_List):
        if (tpr_val + prec_val) > 0:
            f1_val = 2 * tpr_val * prec_val / (tpr_val + prec_val)
        else:
            f1_val = 0
    ...
    return roc_numerator, prc_numerator, avg_f1
...
def build_roc_prc_metric(threshold=None, dataroot=None, ann_file=None, save_path=None, **kwargs):
    ...
    multi_process_score(out_name='roc_prc.csv', threshold=threshold,
                        label_path=os.path.join(dataroot, label_name),
                        save_path=os.path.join('.', save_path))

    roc_numerator, prc_numerator, avg_f1 = roc_prc(save_path)
    return roc_numerator, prc_numerator, avg_f1
